...
ON_COMMAND(ID_Otsu, &CImage_ProcessingView::OnOtsu)
ON_WM_RBUTTONDOWN()
ON_WM_CREATE()
ON_COMMAND(ID_MediaCUDA, &CImage_ProcessingView::OnMediacuda)
ON_COMMAND(ID_32902, &CImage_ProcessingView::On32902)
ON_COMMAND(ID_32903, &CImage_ProcessingView::On32903)
...
void CImage_ProcessingView::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	{
		CDC *pDC = GetDC();
		CBrush *pBrush=CBrush::FromHandle((HBRUSH)GetStockObject(NULL_BRUSH));//空画刷，只画出边框
		CBrush *pOldBrush= pDC->SelectObject(pBrush);
		pDC->SetROP2(R2_NOT);//取反颜色绘制,连续绘制两次就恢复原图

		if (!(MouseX2==0&&MouseY2==0))
			pDC->Rectangle(CRect(CPoint(MouseX,MouseY),CPoint(MouseX2,MouseY2)));//重复上一次所画
...
void CImage_ProcessingView::OnCanny()
{
	...
	m_Image.Flag=0;	//恢复FLAG
	using namespace std;	//使用容器
	CImage_ProcessingView::OnGaussianfilter();//高斯平滑
	//CImage_ProcessingView::OnGaussianfilter();//高斯平滑
	
	//创建新二维数组，用于暂存新图！
	vector<vector<float>> M(w,vector<float>(h)),Mtmp(w,vector<float>(h)),sita(w,vector<float>(h));//梯度，x偏导，y偏导，角度
	set<pair<int,int>> Hsave,Htmp;//保存强边界,新添加的强边界
	float gx(0),gy(0);
	for (auto i=1;i<w-1;++i)
	{
		for (auto j=1;j<h-1;++j)
		{
			gx = std::move((m_Image.m_pBits[0][j+1][i-1] +2 * m_Image.m_pBits[0][j+1][i]+m_Image.m_pBits[0][j+1][i+1])-(m_Image.m_pBits[0][j-1][i-1]+2*m_Image.m_pBits[0][j-1][i]+m_Image.m_pBits[0][j-1][i+1])) ;
			gy = std::move((m_Image.m_pBits[0][j-1][i+1] +2 * m_Image.m_pBits[0][j][i+1]+m_Image.m_pBits[0][j+1][i+1])-(m_Image.m_pBits[0][j-1][i-1]+2*m_Image.m_pBits[0][j][i-1]+m_Image.m_pBits[0][j+1][i-1]));
			M[i][j]  = std::move(sqrt(gx*gx+gy*gy));
			sita[i][j]=std::move(atan(gy/gx)*180/3.1416);
		}
		((CMainFrame *)GetParent())->m_progress.SetPos(((double)i/(double)(w-1)*30));//设置进度条
	}
	...
	for (auto i=1;i<w-1;++i)
	{
		...
		{
			int a(1),b(1),c(1),d(1);
			if ((sita[i][j]>157.5||sita[i][j]<=-157.5)||(sita[i][j]>-22.5&&sita[i][j]<=22.5))//判断方向
			    { a=i;b=j+1;c=i;d=j-1;}
			if ((sita[i][j]>112.5&&sita[i][j]<=157.5)||(sita[i][j]>-67.5&&sita[i][j]<=-22.5))
				{a=i-1;b=j+1;c=i+1;d=j-1;}
			if ((sita[i][j]>67.5&&sita[i][j]<=112.5)||(sita[i][j]>-112.5&&sita[i][j]<=-67.5))
				{a=i-1;b=j;c=i+1;d=j;}
			if ((sita[i][j]>22.5&&sita[i][j]<=67.5)||(sita[i][j]>-157.5&&sita[i][j]<=-112.5))
				{a=i+1;b=j+1;c=i-1;d=j-1;}
			Mtmp[i][j] = (M[i][j]<M[a][b]||M[i][j]<M[c][d]) ? 0:M[i][j];
		}
		((CMainFrame *)GetParent())->m_progress.SetPos(((double)i/(double)(w-1))*20+30);//设置进度条
	}
	M = std::move(Mtmp);
	auto Mmax = M[0][0];
	auto maxiterator = max_element(M.begin(),M.end(),[](const vector<float> & a,const vector<float> &b){ return (*max_element(a.begin(),a.end()))< (*max_element(b.begin(),b.end())); });
	Mmax = *max_element((*maxiterator).begin(),(*maxiterator).end());

	auto H = Mmax*0.03,L = Mmax*0.01; //双阈值
	...
	for (auto i=0;i<w-1;++i)
	{
		for (auto j=0;j<h-1;++j)
			...
			{
				M[i][j] = 255;
				Hsave.insert(std::move(make_pair(i,j)));
			}
			((CMainFrame *)GetParent())->m_progress.SetPos(((double)i/(double)(w-1))*10+50);//设置进度条
	}
	int flag =1,time =0;
	while(flag)
	{
		flag = 0;//么有满足条件的 才可以退出
		for (auto i =Hsave.begin();i!= Hsave.end();++i)
		{
			for (auto k =-2;k<3;++k)
			{
				for(auto kk =-2;k<3;++k)
				{
					if (!((*i).first * (*i).second == 0 ||(*i).first ==1||(*i).second ==1|| (*i).first == w-1||(*i).first == w-2||(*i).second ==h-1||(*i).second ==h-2))//不是边界点
					{
						if(M[(*i).first+k][(*i).second+kk] >L && M[(*i).first+k][(*i).second+kk]!=255 )//是相邻的非强边缘才进入
						{
							flag = 1;//不能退出
							M[(*i).first+k][(*i).second+kk] = 255;
							Htmp.insert(std::move(make_pair((*i).first+k,(*i).second+kk)));
						}
					}
				}
			}
		}
		((CMainFrame *)GetParent())->m_progress.SetPos(min(++time/3,35)+60);//设置进度条
		if (!Htmp.empty())//添加新边界点
		{
			Hsave.insert(Htmp.begin(),Htmp.end());
			Htmp.clear();
		}
	}
	for (auto i=0;i<w-1;++i)
	{
		for (auto j=0;j<h-1;++j)
			m_Image.m_pBits[2][j][i] = m_Image.m_pBits[1][j][i] = m_Image.m_pBits[0][j][i] = M[i][j];
		
		((CMainFrame *)GetParent())->m_progress.SetPos(((double)i/(double)(w-1))*5+95);//设置进度条
	}
	((CMainFrame *)GetParent())->m_progress.SetPos(0);//设置进度条
	Invalidate(TRUE);
}
...
void CImage_ProcessingView::OnGaussianfilter()
{
	...
	double sita = 1.5, N = 9;    //标准差  //尺度

	vector<float> Gx;		// Gy; //Gx == Gy 一维滤波器两个
	for(auto i =0;i<N;++i)
		Gx.push_back( exp((float)-(i-N/2)*(i-N/2)/(2*sita*sita)));
	
	float sum = 0;
	for(auto i =0;i<N;++i) sum += Gx[i];
	sum = sum/(float)N;
	for(auto i =0;i<N;++i) Gx[i] += (1-sum);//均值为1

	//创建新二维数组，用于暂存新图！
	vector<vector<float>> NewPicture(w,vector<float>(h));
	double tmp = 0;
	//行一维卷积
	for (int j=0;j<h;j++)//设定i，j合理的取值范围，防止超出图片范围
	{
		for (int i=((N-1)/2);i<w-((N-1)/2);i++)
		{
		    tmp = 0;
			for (int k=-((N-1)/2);k<(((N-1)/2)+1);k++)//模板内循环
			{
				tmp += (double)m_Image.m_pBits[0][j][i+k] * Gx[k+((N-1)/2)];
			}
			NewPicture[i][j] = tmp/N;
		}
		((CMainFrame *)GetParent())->m_progress.SetPos((double)j/(double)(h-((N-1)/2))*50);//设置进度条
	}
	//将一维行卷积图赋予源图
	for (int j=((N-1)/2);j<h-((N-1)/2);j++)
		for (int i=((N-1)/2);i<w-((N-1)/2);i++)
			m_Image.m_pBits[2][j][i] = m_Image.m_pBits[1][j][i] = m_Image.m_pBits[0][j][i]=NewPicture[i][j];
	//列一维卷积
	for (int j=0;j<w;j++)//设定i，j合理的取值范围，防止超出图片范围
	{
		for (int i=((N-1)/2);i<h-((N-1)/2);i++)
		{
			...
			for (int k=-((N-1)/2);k<(((N-1)/2)+1);k++)//模板内循环
			{
				tmp += (double)m_Image.m_pBits[0][i+k][j] * Gx[k+((N-1)/2)];
			}
			NewPicture[j][i] = tmp/N;
		}
		((CMainFrame *)GetParent())->m_progress.SetPos(((double)j/(double)(h-((N-1)/2)))*50+50);//设置进度条
	}
	((CMainFrame *)GetParent())->m_progress.SetPos(0);//恢复进度条！！
	...
	CImage_ProcessingView::OnGray();
}

//大津算法
void CImage_ProcessingView::OnOtsu()
{
	if(m_Image.IsNull()) return; //判断图像是否为空，如果对空图像进行操作会出现未知的错误
	m_Image.Flag=0;			//恢复FLAG
	using namespace std;	//使用容器
	...
	vector<float> P,SumP,SumMean,Var;//概率、累积概率、累积均值、方差
	int i=0;
	for (auto i=0;i<255;++i)
		P.push_back(m_Image.probability[i]);
	SumMean = SumP = Var = P;
	partial_sum(P.begin(),P.end(),SumP.begin());//累计P
	for_each(P.begin(),P.end(),[&i](float & a){ a = a*(i++); });//计算均值
	partial_sum(P.begin(),P.end(),SumMean.begin());//累计均值
	auto MG = *(--SumMean.end());//MG
	for (auto i=0;i<255;++i)
		Var[i] = (MG*SumP[i] - SumMean[i])*(MG*SumP[i] - SumMean[i])/(SumP[i]*(1-SumP[i]));//计算方差
	auto max = *max_element(Var.begin(),Var.end());
	auto k = find(Var.begin(),Var.end(),max) - Var.begin();//计算阈值
	for (auto i=0;i<w;++i)
		for (auto j=0;j<h;++j)               //二值化
			m_Image.m_pBits[2][j][i] = m_Image.m_pBits[1][j][i] = m_Image.m_pBits[0][j][i] = (m_Image.m_pBits[0][j][i]>k ? 255:0);
	Invalidate(TRUE);
}
...
void CImage_ProcessingView::OnRButtonDown(UINT nFlags, CPoint point)
{
	...
	CMenu menu;
	CMenu *pSubMenu;
	menu.LoadMenuA(IDR_POPUP_EDIT);//自定义右键菜单
...
//中值滤波--CUDA加速版
//extern "C" cudaError_t addWithCuda(BYTE *** a, BYTE *** b,  unsigned int w, unsigned int h,int model); 
void CImage_ProcessingView::OnMediacuda()
{
...
// 
// 		cudaSetDevice(dev);//选取版本1.3以上的GPU设备
// 		cudaGetDeviceProperties(&prop,dev);
// 
// 		auto deviceOverlap = prop.deviceOverlap;
// 
// 
// 		cudaError_t cudaStatus = addWithCuda( NewPicture,m_Image.m_pBits ,w,h,(dlg.m_FileterSize));  
// 
// 		if (cudaStatus != cudaSuccess) 
// 		{  
// 			AfxMessageBox("addWithCuda failed!");
// 		}
...
// 		if (cudaStatus != cudaSuccess) 
// 		{  
// 			AfxMessageBox( "cudaDeviceReset failed!");
// 		}  
// ///////////////////////////////////////
// //将新图赋予源图
// 		for (int j=((dlg.m_FileterSize-1)/2);j<h-((dlg.m_FileterSize-1)/2);j++)//新图是没有初始化的!
...
void CImage_ProcessingView::On32902()
{
	if(m_Image.IsNull()) //判断图像是否为空，如果对空图像进行操作会出现未知的错误
	{
		OnFileOpen();
...
void CImage_ProcessingView::On32903()
{
// 	//保存图片的位置
// 	if (m_Image.IsNull()) {
// 		MessageBox(_T("你还没有打开一个要保存的图像文件！"));
...
// 	int zdistance;
// 	int Distance(0);//球心距离
// 	std::vector<std::vector<int>> SphereCentre;//球心
// 	std::vector<int> radius;//球半径
// 	std::vector<int> core;
// 
// 
// 
// 
// 	std::vector<std::vector<std::vector<int>>> pictures;//图组
// 	std::vector<std::vector<int>> picture;
// 	std::vector<int> line;
// 	for (int i=0;i<w;i++)//对每一个像素点
// 	{
// 		line.push_back(0);
// 	}
// 	for (int j=0;j<h;j++)
...
// 	for(int i =0;i<Num ;++i)
// 	{ 
// 		core.push_back((w-2*R)*rand()/32767.0 + R);//x
// 		core.push_back((h-2*R)*rand()/32767.0 + R);//y
// 		core.push_back((w-2*R)*rand()/32767.0 + R);//z
// 		SphereCentre.push_back(core);
// 		radius.push_back(R*rand()/32767.0);
// 		core.clear();
// 	}
// 
// 	for(int i =0;i <Num;++i)//使得任意两球不相交
// 	{
// 		for(int j =0;j<i;++j)
// 		{
// 			while(1)
// 			{
// 				xdistance = (SphereCentre.at(i).at(0)-SphereCentre.at(j).at(0));
// 				ydistance = (SphereCentre.at(i).at(1)-SphereCentre.at(j).at(1));
// 				zdistance = (SphereCentre.at(i).at(2)-SphereCentre.at(j).at(2));
// 				Distance = xdistance*xdistance + ydistance*ydistance + zdistance*zdistance;
// 				Distance = pow(Distance,0.5);
// 				if(Distance >= radius.at(i)+radius.at(j))
// 				{
// 					break;
// 				}
// 				else
...
// 			for (int i=0;i<w;i++)//对每一个像素点
// 			{
// 				for (int k =0;k<Num;++k)//对每个球     判断（i，j，z）是否在球内
// 				{
// 					int xdistance2(0);
// 					int ydistance2(0);
// 					int zdistance2(0);
// 					int Distance2(0);//球心距离
// 					xdistance2 = i - SphereCentre.at(k).at(0);
// 					ydistance2 = j - SphereCentre.at(k).at(1);
// 					zdistance2 = z - SphereCentre.at(k).at(2);
// 					Distance2 = xdistance2*xdistance2 + ydistance2*ydistance2 + zdistance2*zdistance2;
// 					Distance2 = pow(Distance2,0.5);
// 
// 					if (Distance2 < radius.at(k))
// 					{
// 						pictures.at(z).at(i).at(j)=255;
// // 						NewPicture[0][j][i] = 255;
// // 						NewPicture[1][j][i] = 255;
// // 						NewPicture[2][j][i] = 255;
// 					}
// 				}							
// 			}
// 		}
// 		//Update(NewPicture);
// 		//Save(z,strFileName,strExtension);
// 	}
// 
// for(int z = 0;z<w;++z)//对每一张图片
// {
...
// 		for (int i=0;i<w;i++)//对每一个像素点
// 		{
// 			m_Image.m_pBits[0][j][i]=pictures.at(z).at(i).at(j);
// 			m_Image.m_pBits[1][j][i]=pictures.at(z).at(i).at(j);
// 			m_Image.m_pBits[2][j][i]=pictures.at(z).at(i).at(j);
// 		}
// 	}
// 	Save(z,strFileName,strExtension);
// }
...
extern "C" cudaError_t addWithCuda(int w,int h,int start,int end,int Num,int* Sphere_Center,int *Radius_ ,int * PictureBlock);
void CImage_ProcessingView::On32904()
{
	...
	if( w>1024 )
	{
		MessageBox(_T("图片过大！块最大线程数1024：请选择宽度小于1024的图片。"));
		return;
	}
	...
	const int Num = 100;//球个数 ：若球数和图像尺寸同时过大 cuda核函数会崩溃
	int R = 50;//最大半径
	int xdistance;
	int ydistance;
	int zdistance;
	...
	//转一维数组
	int Sphere_Center [Num*3];
	int Radius_ [Num];
	for(int i =0;i<Num ;++i)
	{ 
		Sphere_Center[3*i] = SphereCentre[i][0];
		Sphere_Center[3*i+1] = SphereCentre[i][1];
		Sphere_Center[3*i+2] = SphereCentre[i][2];
		Radius_[i] = radius[i];
	}
///////////////////////////////////////////////////////cuda
	cudaDeviceProp prop;
	...
	auto deviceOverlap = prop.deviceOverlap;
	///////////////////////////////////////////////////////////////////
	double GlobalMen = prop.totalGlobalMem;	//最大显存
	double PicNum;							//图片张数
	int * malloctest;
	int percent=6;									//可分配内存占总显存比例
	PicNum =(int) (GlobalMen/percent/w/h/4);		//一次生成最大图片张数
	malloctest = (int*)malloc(PicNum*w*h*sizeof(int));//防止内存不够，无法malloc
	while(!malloctest)
	{
		++percent ;
		PicNum =(int) (GlobalMen/percent/w/h/4);
		malloctest = (int*)malloc(PicNum*w*h*sizeof(int));
	}
	free(malloctest);
	///////////////////////////////////////////////////////////
	...
	int start(0),end(0);
	for(start =0 ;start < w ; start +=PicNum)//共w张，分批次处理；每次PicNum张
	{
		end = (start + PicNum-1) <w ? (start + PicNum-1) : w-1;
		int RealPicNum = end-start+1;

		int * PictureBlock;
		PictureBlock = (int*)malloc(RealPicNum*w*h*sizeof(int));
		if(!PictureBlock)
		{ AfxMessageBox("malloc failed!");
			return;
		}
		
		cudaStatus = addWithCuda(w,h,start,end,Num,Sphere_Center,Radius_,PictureBlock);  

		if (cudaStatus != cudaSuccess) 
			AfxMessageBox("addWithCuda failed!");

		for(int z = start; z <= end; ++z)//对每一张图片存储
		{
			for (int j=0;j<h;j++)
			{
				for (int i=0;i<w;i++)
				{
					m_Image.m_pBits[0][j][i]=PictureBlock[(z-start)*w*h + j*w +i];
					m_Image.m_pBits[1][j][i]=PictureBlock[(z-start)*w*h + j*w +i];
					m_Image.m_pBits[2][j][i]=PictureBlock[(z-start)*w*h + j*w +i];
				}
			}
			Save(z,strFileName,strExtension);
		}
	...
	ofstream o_file;
	CString filepath;
	filepath = strFileName + _T(".txt");
	o_file.open(filepath,ios::trunc|ios::out);

	for (int i=0 ;i<Num;++i)
	{
		o_file<<radius.at(i)<<_T("\n");
	}
	o_file.clear();//重置状态标志
	o_file.close();
...
void CImage_ProcessingView::Save(int b,CString strFileName,CString strExtension)
{
	CString a;
	a.Format("%d", b);
	strFileName = strFileName + a + _T(".") + strExtension;
	//	//strFileName="C:\\Users\\Lenovo\\Desktop\\QSanguosha-Qingming\\abc.bmp";
	//}
	// 图像保存
	HRESULT hResult;
